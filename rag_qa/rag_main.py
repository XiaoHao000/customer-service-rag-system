...
def main(query_mode=True, directory_path="data"):
    ...
    if not query_mode:
        ...
        while True:
            ...
            source_filter_input = input(f"请输入产品线 ({'/'.join(valid_sources)}) (直接回车默认不过滤)：").strip()
            source_filter = None  # 默认不过滤
            if source_filter_input:
                if source_filter_input in valid_sources:
                    source_filter = source_filter_input
                    logger.info(f"用户选择了产品线过滤: {source_filter}")
                else:
                    logger.warning(
                        f"无效的产品线 '{source_filter_input}'，将不过滤"
                    )
                    print(f"提示：输入的产品线 '{source_filter_input}' 无效，将不过滤。")
...
if __name__ == "__main__":
    ...
    # import argparse
    #
    # parser = argparse.ArgumentParser(description="智能客服系统主入口")
