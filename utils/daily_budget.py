...
设计决策：
- 按 IP 维度计数——每个用户独立 30 次额度，互相不影响。key 含项目前缀 rag-qa，三个项目额度独立
- Redis 计数器而非内存——多 worker 共享，Docker 重启不丢计数
- 只计 /api/query（HTTP POST）—— WebSocket /api/stream 需在 handler 内单独计数
...
# 需要计数的 API 路径（注：WebSocket /api/stream 不走 HTTP 中间件，需单独处理）
_COUNTED_PATHS = {"/api/query"}


class DailyBudgetMiddleware(BaseHTTPMiddleware):
    """每日演示额度中间件。

    用 Redis INCR 维护当日查询计数，达到上限后拒绝请求。
    """
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        key = f"demo:usage:rag-qa:{ip}:{today}"
...
async def check_ws_quota(redis_url: str, ip: str, max_queries: int = 30) -> tuple[bool, int]:
    """WebSocket 专用额度检查（BaseHTTPMiddleware 无法拦截 WebSocket 连接）。

    返回 (allowed, count)：allowed=False 表示超限应拒绝连接。
    """
    try:
        import redis.asyncio as aioredis
        ...
        logger.warning(f"WebSocket 额度检查 Redis 连接失败，fail-open 放行: {e}")
        return True, 0
    ...
    try:
        ...
        if count > max_queries:
            logger.warning(f"WebSocket 每日额度已用完: ip={ip} {count}/{max_queries}")
            return False, count

        logger.debug(f"WebSocket 每日额度: ip={ip} {count}/{max_queries}")
        return True, count
    except Exception as e:
        logger.warning(f"WebSocket 额度检查异常，fail-open 放行: {e}")
        return True, 0
    finally:
        await r.aclose()
